...
**技术实现**：
- 文件：`backend/app/services/agent_service.py` (338 行)
- SSE 流式输出：9 种事件类型
- 测试覆盖：67 项单元测试
...
- 知识提取：`backend/app/services/knowledge_service.py` (675 行)
...
### 2.5 插件引擎 2.0 — 会话模式（Phase 1 已完成）

**创新点**：插件从"一次性请求-响应"升级为**持久会话模式**，支持 WebSocket 双向通信，用户可在同一连接中连续操作。
...
2. **插件引擎 2.0 Phase 2**（自定义 UI）
   - 插件注册自定义前端组件
   - 不再局限于表格输出

### 中期（3-6 个月）

3. **插件引擎 2.0 Phase 3**（插件编排）
   - DAG 工作流引擎
   - 可视化编排器
   - 插件串联自动化
...
4. **插件引擎 2.0** — 会话模式，WebSocket 双向通信，持久状态
5. **沙箱终端** — Docker 容器隔离 + 黑名单过滤

**技术栈现代化**：Next.js 16 + FastAPI + PostgreSQL + pgvector + Redis + Docker

**测试覆盖充分**：286 项单元测试，核心模块 60%+ 覆盖率

**国际化完善**：8 种语言，3872 条翻译
