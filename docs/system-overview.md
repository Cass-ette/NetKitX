...
**创新点**：Agent 会话自动转换为可视化 DAG 工作流，逐步回放时 AI 对每步执行结果进行反思分析。
...
**技术实现**：
- 工作流服务：`backend/app/services/workflow_service.py`
- 回放 API：`backend/app/api/v1/workflows.py`（SSE + AI 反思）
