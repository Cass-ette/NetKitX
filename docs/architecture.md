...
│   │   │   ├── terminal/      # xterm.js 内嵌终端
│   │   │   ├── topology/      # 网络拓扑 (React Flow + dagre)
│   │   │   └── viz/           # 可视化 (网络图, 图表)
│   │   ├── lib/
...
│   │   │       ├── auth.py
│   │   │       ├── tools.py
│   │   │       ├── tasks.py    # 含 /logs 历史日志端点
│   │   │       ├── plugins.py
│   │   │       ├── marketplace.py
│   │   │       ├── reports.py  # 报告导出 (HTML/PDF)
│   │   │       └── topology.py # 网络拓扑数据
│   │   ├── core/
│   │   │   ├── config.py      # 全局配置
...
### Phase 3: 扩展增强 ✅
- 插件市场 — 发布/搜索/安装，PubGrub 依赖解析，安全扫描
- 报告导出 — Jinja2 + WeasyPrint 生成 HTML/PDF，含参数、结果表格和统计
- 内嵌终端 — xterm.js 只读终端，历史日志 + WebSocket 实时流
- 网络拓扑可视化 — React Flow + dagre 自动布局，Scanner/Host 节点，详情面板
